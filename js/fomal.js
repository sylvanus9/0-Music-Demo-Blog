...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Sylvanus9🍹")[0];
}
...
function createtime2() {
    ...
    setTimeout(
        console.warn.bind(
            console,
            "%c ⚡ Powered by Sylvanus🍹 %c 你正在访问Sylvanus🍹の小窝",
...
function share_() {
  ...
  try {
      ...
      var subTitle = title.endsWith("| sylvanus🍹") ? title.substring(0, title.length - 14) : title;
      navigator.clipboard.writeText('sylvanus🍹的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
      new Vue({
          data: function () {
              this.$notify({
                  title: "成功复制分享链接🎉",
                  message: "您可以通过粘贴分享给小伙伴啦！",
                  position: 'top-left',
                  offset: 50,
                  showClose: true,
                  type: "success", 
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("01/18/2023 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `  <div style="font-size:13px;font-weight:bold">本站已经坚挺了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 ⛄</div>`
      : `  <div style="font-size:13px;font-weight:bold">本站已经坚挺了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 ⛄</div>`),
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">已臻蜗牛境🐌</span>`
      } else if (fps <= 15) {
          var kd = `<span style="color:red">直追蠕动者🐛</span>`
      } else if (fps <= 25) {
          var kd = `<span style="color:orange">嗡嗡,裂开🐝</span>`
      } else if (fps < 35) {
          var kd = `<span style="color:#9338e6">差强人意砸🐄</span>`
      } else if (fps <= 45) {
          var kd = `<span style="color:#08b7e4">还不错呦🦘</span>`
      } else {
          var kd = `<span style="color:#39c5bb">登峰造极喽🐆</span>`
      }
